...
export default function ChartMotion({ children, motionKey, direction = 'horizontal', decorative = false }: {
  ...
  return <div ref={root} className="chart-motion" data-motion-state={reducedMotion ? 'reduced' : 'waiting'} data-testid="chart-motion" role={decorative ? undefined : 'group'} aria-label={decorative ? undefined : 'Data chart'}>{children}</div>
}
